...
from PyQt6.QtWidgets import (
    ...
    QFileDialog, QGraphicsView, QScrollArea, QProgressBar
)
...
class NodeEditorWindow(QMainWindow):
    ...
    def initStatusBar(self):
        """Create a status bar with a progress bar for build progress."""
        self.status_bar = self.statusBar()
        self.progress_bar = QProgressBar()
        self.progress_bar.setValue(0)
        self.progress_bar.setMaximum(1)
        self.status_bar.addPermanentWidget(self.progress_bar)
        self.total_nodes_to_build = 0
        self.built_nodes = 0
    ...
    def onBuildAll(self, start_node_name=None, force_first=False):
        ...
        self.total_nodes_to_build = len(project_commands.node_commands_list)
        self.built_nodes = 0
    ...
    def onWorkerResponse(self, respData: SubprocessResponseData):
        ...
        if respData.index != -1:
            if respData.result:
                self.built_nodes += 1
                self.progress_bar.setValue(self.built_nodes)
            else:
                # mark progress but indicate failure
                self.built_nodes += 1
                self.progress_bar.setValue(self.built_nodes)
